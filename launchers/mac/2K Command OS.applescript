...
on configureEnv(projectDir)
    ...
    set enabledChoice to button returned of (display dialog "Ativar Discord Bot?" with title appName buttons {"Não", "Sim"} default button (if currentEnabled is "true" then "Sim" else "Não"))
...
on openBackend(projectDir)
    ...
    do shell script "cd " & quoted form of (my serverDir(projectDir)) & " && nohup npm run dev > backend.log 2>&1 &"
...
on openFrontend(projectDir)
    do shell script "cd " & quoted form of (my webDir(projectDir)) & " && nohup npm run dev > frontend.log 2>&1 &"
...
on testStatus()
    ...
    try
        ...
        if botResponse contains "\"ready\":true" then
